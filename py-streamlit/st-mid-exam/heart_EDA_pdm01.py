...
st.sidebar.subheader('Visualization using seaborn and plotly by pdm01')
...
df['Outcome'] = df['target'].apply(lambda x: 'HD' if x == 1 else 'noHD')
...
hmap_hm = df["thal", "thalach", "exang", "oldpeak", "slope", "ca", "thal"]
#df.map = df.corr({0: 'age', 1: 'cp', 2: 'thalach', 3: 'exchang', 4: 'oldpeak', 5: 'slope', 6: 'ca', 7: 'thal'})
fig6 = plt.figure(figsize=(12,10))
sns.heatmap(df[hmap_hm].corr(),annot=True, vmin=-1, vmax=1, cmap='coolwarm')
st.pyplot(fig6)
